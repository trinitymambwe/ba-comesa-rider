  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #1a73e8 0%, #1557b0 25%, #0d1b2a 50%, #0d1b2a 100%)', color: 'white' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', margin: '0 20px 16px', backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '14px', padding: '4px' }}>
        {[
          { key: 'my' as const, icon: '📦', label: `My Deliveries (${deliveries.length})` },
          { key: 'available' as const, icon: '💰', label: 'Earnings' },
        ].map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '12px', backgroundColor: activeTab === tab.key ? 'rgba(255,255,255,0.2)' : 'transparent', color: activeTab === tab.key ? 'white' : 'rgba(255,255,255,0.5)' }}>{tab.icon} {tab.label}</button>
        ))}
      </div>
        ...
        {activeTab === 'my' && (
          deliveries.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', borderRadius: '20px', backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px' }}>Admin will assign orders to you.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {deliveries.map((d: any) => (
                <div key={d.id} style={{ padding: '20px', borderRadius: '16px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)' }}>Seller: {d.sellerName} · 📱 {d.sellerPhone}</p>
